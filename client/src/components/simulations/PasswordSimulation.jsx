...
function PasswordSimulation({ simulation, onComplete }) {
  ...
  const handlePasswordCreated = (password, strength) => {
    setUserPassword(password);
    setPasswordStrength(strength);
    setCurrentStep('bruteforce');
  };

  const handleBruteForceComplete = () => {
    setCurrentStep('leakcheck');
  };

  const handleLeakCheckComplete = () => {
    if (passwordStrength < 60) {
      setCurrentStep('hacked');
    } else {
          ...
          <div className="flex gap-1 mb-8">
            {[0, 1, 2, 3].map((i) => (
              <motion.div
                ...
                style={{ background: ['#f25022', '#7fba00', '#00a4ef', '#ffb900'][i] }}
  ...
  return (
    <div className="relative w-full h-full overflow-hidden">
      ...
      <div className="absolute top-4 left-4 flex flex-col gap-4 z-10">
        <DesktopIcon label="Браузер" emoji="🌐" onClick={() => setCurrentStep('intro')} />
      ...
      <div className="absolute inset-0 flex items-center justify-center z-20">
        <AnimatePresence mode="wait">
          {currentStep === 'intro' && (
            <IntroStep onStart={() => setCurrentStep('generator')} />
          )}
          {currentStep === 'generator' && (
            <PasswordGenerator
              commonPasswords={simulation.commonPasswords}
              onComplete={handlePasswordCreated}
            />
          )}
          {currentStep === 'bruteforce' && (
            <BruteForceAnimation
              password={userPassword}
              strength={passwordStrength}
              onComplete={handleBruteForceComplete}
            />
          )}
          {currentStep === 'leakcheck' && (
            <LeakCheck
              password={userPassword}
              commonPasswords={simulation.commonPasswords}
              onComplete={handleLeakCheckComplete}
            />
          )}
          {currentStep === 'hacked' && (
            <AccountHacked
              scenario={simulation.scenarios.find(s => s.type === 'hacked')}
              onRetry={() => setCurrentStep('generator')}
              onContinue={() => setCurrentStep('manager')}
            />
          )}
          {currentStep === 'manager' && (
            <PasswordManager onComplete={handleManagerComplete} />
          )}
          {currentStep === 'quiz' && (
            <QuizStep
...
function IntroStep({ onStart }) {
  return (
    ...
    >
      <h2 className="text-2xl font-bold text-white mb-4">🔐 Симуляция: Создание пароля</h2>
      <p className="text-gray-300 mb-6 leading-relaxed">
        В этой симуляции вы научитесь создавать надёжные пароли и поймёте, почему это важно.
      </p>
      <div className="space-y-3 mb-6">
        <div className="flex items-start gap-3 p-3 bg-[#0a0a0f] rounded-lg">
          <span className="text-xl">1️⃣</span>
          <div>
            <div className="text-white font-medium">Создайте пароль</div>
            <div className="text-gray-400 text-sm">Попробуйте создать надёжный пароль</div>
          </div>
        </div>
        <div className="flex items-start gap-3 p-3 bg-[#0a0a0f] rounded-lg">
          <span className="text-xl">2️⃣</span>
          <div>
            <div className="text-white font-medium">Увидьте атаку перебором</div>
            <div className="text-gray-400 text-sm">Посмотрите, как хакер пытается взломать ваш пароль</div>
          </div>
        </div>
        <div className="flex items-start gap-3 p-3 bg-[#0a0a0f] rounded-lg">
          <span className="text-xl">3️⃣</span>
          <div>
            <div className="text-white font-medium">Проверка утечек</div>
            <div className="text-gray-400 text-sm">Узнайте, есть ли ваш пароль в базах утечек</div>
          </div>
        </div>
        <div className="flex items-start gap-3 p-3 bg-[#0a0a0f] rounded-lg">
          <span className="text-xl">4️⃣</span>
          <div>
            <div className="text-white font-medium">Менеджер паролей</div>
            <div className="text-gray-400 text-sm">Научитесь использовать Bitwarden</div>
          </div>
        </div>
      </div>
      <motion.button
...
function ResultsStep({ password, strength, onRestart, onFinish }) {
  const getGrade = () => {
    if (strength >= 80) return { emoji: '🏆', text: 'Эксперт по паролям!', color: '#22c55e' };
    if (strength >= 60) return { emoji: '', text: 'Хороший результат!', color: '#eab308' };
    return { emoji: '📚', text: 'Нужно больше практики', color: '#ef4444' };
  };
  ...
  return (
    <motion.div
      className="bg-[#1a1a2e] rounded-xl w-full max-w-2xl p-8 border border-gray-700 text-center"
      initial={{ scale: 0.9, opacity: 0 }}
      animate={{ scale: 1, opacity: 1 }}
      exit={{ scale: 0.9, opacity: 0 }}
    >
      ...
      <p className="text-gray-400 mb-6">
        Ваш пароль: <span className="font-mono text-white">{password || 'не создан'}</span>
      </p>
      <div className="mb-6">
        <div className="text-sm text-gray-400 mb-2">Надёжность пароля</div>
        ...
        <div className="text-right text-sm mt-1" style={{ color: grade.color }}>{strength}%</div>
      </div>
      <div className="flex gap-3">
        <motion.button
          className="flex-1 py-3 rounded-lg font-medium text-white"
          style={{ background: 'linear-gradient(to bottom, #6b7280 0%, #4b5563 100%)' }}
          whileHover={{ scale: 1.02 }}
          whileTap={{ scale: 0.98 }}
          onClick={onRestart}
        >
